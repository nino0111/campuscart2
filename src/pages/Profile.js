...
import { auth, db } from "../firebase"; // ✅ Removed storage, using Cloudinary
...
import { 
  ...
  Star, CheckCircle2, TrendingUp, Loader2
} from "lucide-react";
...
export default function Profile() {
  ...
  return (
    <div className="home-container" style={{ background: COLORS.bgLight, minHeight: '100vh' }}>
        ...
        <aside style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div style={{ background: 'white', borderRadius: '24px', padding: '30px', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', textAlign: 'center', border: '1px solid #E2E8F0' }}>
            <div style={{ position: 'relative', width: '100px', height: '100px', margin: '0 auto 15px' }}>
              <div style={{ width: '100%', height: '100%', borderRadius: '50%', background: '#F1F5F9', overflow: 'hidden', border: '3px solid white', boxShadow: '0 4px 10px rgba(0,0,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {isUploading ? (
                  <Loader2 className="animate-spin" size={30} color={COLORS.primary} />
                ) : user.photoURL ? (
                  <img src={user.photoURL} alt="pfp" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                ) : (
              ...
              <label htmlFor="fileInput" style={{ position: 'absolute', bottom: '0', right: '0', background: COLORS.primary, padding: '6px', borderRadius: '50%', cursor: 'pointer', color: 'white' }}>
                <Camera size={14} />
                <input type="file" id="fileInput" hidden onChange={handleImageChange} accept="image/*" disabled={isUploading} />
              </label>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <button onClick={handleLogout} style={{ ...sidebarBtnStyle, color: '#E11D48', background: '#FFF1F2', border: '1px solid #FECDD3' }}>
      ...
      {/* ✅ CUSTOM ALERT MODAL - SAME BRANDED DESIGN */}
      {showAlert && (
        <div style={{
          position: "fixed",
          top: 0,
